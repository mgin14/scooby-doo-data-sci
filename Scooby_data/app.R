...
scoobydoo_tidy <- scoobydoo_tidy %>% 
  mutate(
    caught_by = str_match(caught_by, pattern = "fred|daphnie|velma|shaggy|scooby")
  )

scoobydoo_tidy <- scoobydoo_tidy %>% 
  mutate(
    date_aired = year(date_aired)
  )
...
server <- function(input, output) {
    ...
    #Who has caught the culprit the most throughout the series?
